...
other" (they are thirty tiles apart), and "creeping over the lip of the drop and
then driving on perfectly well" (one of them does creep at the lip, and does not
then drive on).

What they actually do is **oscillate at the foot of a ramp**, for the rest of
the race. Traced tick by tick, one of them alternates `accel right` and
`brake left` every second or two, its heading flicking between 58845 and 58998,
its position jittering by hundredths of a tile around 46.49, 14.55, and its
speed bouncing between 0.001 and 0.159 tiles a second. The driver decides it is
against something it cannot climb and backs off; a hair further back it decides
the way is clear and accelerates; the ramp stops it again. Neither car escapes
and neither completes a lap.

The code that reverses out of a wall already knows about this shape - its
comment says a car backing off with the wheel the same way "comes back off the
wall pointing at it again, which is an oscillation rather than a recovery" - and
turns the wheel the other way for exactly that reason. That is not enough here.
What would settle it is hysteresis: a car already backing away should keep
backing away until it is clear by a margin, rather than re-deciding at the
boundary every tick. Whether that belongs in the driver, and what it should do
once clear, is a question about how an opponent ought to drive.

**Three wrong descriptions is the lesson worth keeping.** Each was written from
a partial look - a diagnostic that reported one car when two were failing, a
snapshot at one moment, a trace that stopped before the car came back. The
account above is from watching both cars to the end of the race and printing
what the driver asked for on every sample.
