    ...
    // is in trouble leaves the route intact along the other one.
    {
        ...
        // behind it.
        {
            ...
            if (reach > 0) {
                ...
                // **And it asks the whole way up, not one point of it.**
                //
                // At a tile and a half only, a hill that begins gently and
                // steepens after it answers "climbable" at exactly the distance
                // the question is put. That is not a near miss but a trap that
                // holds: the car creeps up the gentle part, comes to rest on the
                // steep part, and from there the same tile and a half ahead is
                // gentle again. Two were found on `the big one` at nine
                // hundredths of a tile a second with the throttle wide open,
