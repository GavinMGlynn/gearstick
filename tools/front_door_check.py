...
def run_client(game, port, key, screen, env, seconds, stop_on_racing):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            while time.monotonic() < deadline:
                ...
                if not line:
                    if server.poll() is not None:
                        break
                    continue
                found = re.search(r"key ([0-9a-f]{64})", line)
                if found:
                    key = found.group(1)
                    break
            if key is None:
            ...
            racing, log = run_client(game_bin, port, key, "title", env,
                                     MENU_SETTLE_SECONDS, stop_on_racing=True)
            if racing:
